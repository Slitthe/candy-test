...
const Home = (props: HomeProps) => {
    ...
    console.log({ itemsRedeemed, itemsRemaining, itemsAvailable })

    const [nextItem, setNextItem] = useState<any>(null)
    async function getCandyInfo() {
        if (candyMachine?.id) {
            const address = candyMachine.id.toString()

            const res = await fetch(
                `https://europe-west1-anft-6f8f4.cloudfunctions.net/getCandyMachineInfo?address=${address}`,
            )
            ...
            console.log({
                resJson,
                items: resJson.items,
                itemsMinted: resJson.itemsMinted,
            })

            const nextItemIndex = candyMachine.state.itemsRedeemed
            console.log({ nextItemIndex })
            const metadataUri = resJson.items[nextItemIndex].uri

            const metadataRes = await fetch(metadataUri)
            const metadataResJson = await metadataRes.json()

            setNextItem(metadataResJson)
            console.log({ metadataResJson })
            // const resJson = JSON.parse(resText)
        }
    }
